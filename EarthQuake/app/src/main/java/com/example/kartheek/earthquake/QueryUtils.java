...
public class QueryUtils {
    ...
    static public ArrayList<Earthquake> getEarthQuakes(String murl)
    {
        ...
        try {
            ...
            Log.e("EarthquakeActivity","Error in making http request", e);
        }
        if(json!=null) {

            Log.e("QueryUtils.class",json);
            Log.e("QueryUtils.class", "fetching the earthquake data");
            //to store the different earthquakes occurred
            ...
            try {
                ...
                //a new JSON object to parse the above JSON data
                JSONObject jEarthQuake = new JSONObject(json);

                //JSON array to store the earthquakes features in the data
                JSONArray earthQuakeFeatures = jEarthQuake.optJSONArray("features");

                //loop to fill the earthquakes array list with earthquakes data given above
                for (int i = 0; i < earthQuakeFeatures.length(); i++) {

                    //splitting the place statement into two strings
                    //first string the direction and distance of the earthquake from the given city
                    //second string is the name of the city in which earthquake occurred
                    CharSequence place = earthQuakeFeatures.getJSONObject(i).getJSONObject("properties").getString("place");
                    String primaryPlace = "";
                    String secondaryPlace = "";
                    int j;
                    for (j = 0; j < place.length(); j++) {
                        primaryPlace += "" + place.charAt(j);
                        if (j > 1 & j < (place.length() - 1)) {
                            if (place.charAt(j - 2) == ' ' & place.charAt(j - 1) == 'o' & place.charAt(j) == 'f' & place.charAt(j + 1) == ' ') {
                                break;
                            }
                        }
                    }
                    if (!(j < place.length())) {
                        primaryPlace = "Near the";
                        secondaryPlace += place + "";
                    } else
                        for (j = j + 2; j < place.length(); j++)
                            secondaryPlace += "" + place.charAt(j);

                    //to format the magnitude of double type to get the magnitude upto one decimal place
                    int mag = (int) (earthQuakeFeatures.getJSONObject(i).getJSONObject("properties").getDouble("mag") * 10);
                    double formattedMag = (double) mag / (double) 10;

                    earthquakes.add(new Earthquake(formattedMag, primaryPlace, secondaryPlace, (new SimpleDateFormat("LLL dd, yyyy").format(new Date(earthQuakeFeatures.getJSONObject(i).getJSONObject("properties").getLong("time")))) + "", (new SimpleDateFormat("h:mm a").format(new Date(earthQuakeFeatures.getJSONObject(i).getJSONObject("properties").getLong("time")))) + "", earthQuakeFeatures.getJSONObject(i).getJSONObject("properties").getString("url")));
                }
    ...
    static public String makeHttpRequest(URL Urls) throws IOException {
        String JsonResponse = "";
        InputStream in = null;
        HttpURLConnection Url = null;
        try {
            Url = (HttpURLConnection) Urls.openConnection();
            ...
            if(Url.getResponseCode() == 200) {
                ...
                Log.e("EarthQuakeActivity","Error in response code");
            }
        } catch (IOException e) {
            Log.e("EarthQuakeActivity","Connection cannot be established",e);
        } finally {
            if(Url !=null){
                Url.disconnect();
            }
    ...
    static public String readFromStream(InputStream in) throws IOException {
        StringBuilder output = new StringBuilder();
        InputStreamReader getInput = new InputStreamReader(in, Charset.forName("UTF-8"));
        BufferedReader input = new BufferedReader(getInput);
    ...
    static public URL createUrl(String url) {
        URL Url = null;
        try {
            Url = new URL(url);
        } catch (MalformedURLException e) {
            Log.e("MainActivity","Error in formming the URL",e);
        }
        return Url;
    }
